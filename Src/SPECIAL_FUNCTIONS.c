...
static double Factorial_DB(int n);
...
extern double Legendre_Hoeksema(int L, int M, double Theta){
    ...
    Legendre *= Factorial_DB(2*L-1)*sqrt(q/Factorial(L-M)/Factorial(L+M)) \
        *pow(sin(Theta), M);
    return Legendre;
}
...
extern void Compute_SB_Zeros(double **Zeros, int Zero_L, int Zero_N, \
    ...
    /*######################################################################
      ...
      Input parameters:
        ...
        Flag_Derivative, If the flag = 1, output the zeros of the 
            derivative, else output the zeros of the function.
        Flag_X01, If the flag = 1, X01 is set to 0 (only effective when
            Flag_Derivative = 1).
      Output parameters:
...
extern void Zeros_Output(char *filename, int Zero_L, int Zero_N, \
    ...
    /*######################################################################
      Purpose:
        write positive zeros of spherical Bessel funtion or of its 
